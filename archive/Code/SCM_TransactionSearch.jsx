...
// ════════════════════════════════════════════════════════
// GLOBAL QUICK SEARCH OVERLAY (Ctrl+K / Cmd+K)
// ════════════════════════════════════════════════════════
const QuickSearchOverlay = ({ isOpen, onClose, onSelectTransaction }) => {
...
export default function SCMTransactionSearch({ onNavigate } = {}) {
  ...
  const [filters, setFilters] = useState({ markStatus: "Tümü", entityType: "", scoreMin: "", scoreMax: "", dateFrom: "", dateTo: "", source: "", amountMin: "", amountMax: "" });
  ...
  useEffect(() => {
    const handler = (e) => { if ((e.metaKey || e.ctrlKey) && e.key === "k") { e.preventDefault(); setQuickSearchOpen(prev => !prev); } };
  ...
  const resetFilters = () => {
    setFilters({ markStatus: "Tümü", entityType: "", scoreMin: "", scoreMax: "", dateFrom: "", dateTo: "", source: "", amountMin: "", amountMax: "" });
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", background: C.bg, fontFamily: "'DM Sans', 'Segoe UI', -apple-system, sans-serif" }}>
      ...
      <QuickSearchOverlay isOpen={quickSearchOpen} onClose={() => setQuickSearchOpen(false)} onSelectTransaction={txn => setDrawerTxn(txn)} />
      ...
      <aside style={{ width: sW, background: C.sidebar, display: "flex", flexDirection: "column", transition: "width 0.3s cubic-bezier(0.4,0,0.2,1)", flexShrink: 0, zIndex: 100, position: "relative" }}>
        ...
        <div style={{ padding: sidebarCollapsed ? "16px 8px" : "16px", borderTop: "1px solid rgba(255,255,255,0.06)" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, justifyContent: sidebarCollapsed ? "center" : "flex-start" }}>
            <div style={{ width: 34, height: 34, borderRadius: "50%", flexShrink: 0, background: "linear-gradient(135deg, #6366F1, #8B5CF6)", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontWeight: 600, fontSize: 13 }}>{user.name.split(" ").map(n => n[0]).join("")}</div>
            {!sidebarCollapsed && <div style={{ flex: 1, overflow: "hidden" }}><div style={{ color: "#E2E8F0", fontSize: 13, fontWeight: 500, whiteSpace: "nowrap", textOverflow: "ellipsis", overflow: "hidden" }}>{user.name}</div><div style={{ color: "#64748B", fontSize: 11 }}>{user.roleLabel}</div></div>}
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        <header style={{ height: 64, background: "#fff", borderBottom: `1px solid ${C.border}`, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 28px", flexShrink: 0 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            {/* ★ Quick Search Button */}
            <button onClick={() => setQuickSearchOpen(true)} style={{ display: "flex", alignItems: "center", gap: 8, padding: "7px 14px", borderRadius: 8, border: `1px solid ${C.border}`, background: "#F8FAFC", cursor: "pointer", color: C.textSecondary, fontSize: 13, transition: "all 0.15s" }}
              onMouseEnter={e => { e.currentTarget.style.borderColor = C.primaryLight; e.currentTarget.style.background = C.primaryBg; }}
              onMouseLeave={e => { e.currentTarget.style.borderColor = C.border; e.currentTarget.style.background = "#F8FAFC"; }}>
              <Icons.Search />
              <span style={{ fontSize: 12.5, minWidth: 120 }}>Hızlı Arama...</span>
              <kbd style={{ padding: "2px 6px", borderRadius: 4, fontSize: 10, fontWeight: 600, background: "#fff", color: C.textSecondary, border: `1px solid ${C.border}`, fontFamily: "'JetBrains Mono', monospace", marginLeft: 4 }}>{isMac ? "⌘K" : "Ctrl+K"}</kbd>
            </button>
        ...
        <main style={{ flex: 1, overflow: "auto", padding: 28 }} onClick={() => { setShowNotifications(false); setDomainMenuOpen(false); }}>
          {/* Info Banner */}
          <div style={{ background: "linear-gradient(135deg, #EFF6FF 0%, #DBEAFE 100%)", borderRadius: 12, padding: "16px 22px", marginBottom: 20, border: "1px solid #BFDBFE", display: "flex", alignItems: "flex-start", gap: 12 }}>
            ...
            <div style={{ fontSize: 13, color: "#1E3A8A", lineHeight: 1.6 }}>
              ...
              Herhangi bir sayfadan <kbd style={{ padding: "1px 6px", borderRadius: 4, border: "1px solid #93C5FD", background: "#EFF6FF", fontSize: 11.5, fontFamily: "'JetBrains Mono', monospace" }}>{isMac ? "⌘K" : "Ctrl+K"}</kbd> ile hızlı aramaya ulaşabilirsiniz.
            </div>
          </div>
          ...
          <div style={{ background: "#fff", borderRadius: 14, border: `1px solid ${C.border}`, padding: "16px 22px", marginBottom: 16, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}>
            ...
            <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
              ...
              <button style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 14px", borderRadius: 8, border: `1px solid ${C.border}`, background: "#fff", cursor: "pointer", fontSize: 12.5, fontWeight: 500, color: C.textSecondary }}><Icons.Download /> Dışa Aktar</button>
            ...
            <div style={{ background: "#fff", borderRadius: 14, border: `1px solid ${C.border}`, padding: "20px 22px", marginBottom: 16, animation: "fadeSlideDown 0.2s ease" }}>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16 }}>
                ...
                <div>
                  ...
                  <select value={filters.entityType} onChange={e => { setFilters(f => ({ ...f, entityType: e.target.value })); setPage(1); }} style={{ width: "100%", padding: "8px 10px", borderRadius: 8, border: `1px solid ${C.border}`, fontSize: 12.5, outline: "none", background: "#F8FAFC", color: C.text }}>
                    <option value="">Tümü</option><option value="Customer">Customer</option><option value="Account">Account</option><option value="Card">Card</option><option value="Device">Device</option>
                  </select>
                </div>
                <div>

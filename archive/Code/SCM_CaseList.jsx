...
export default function SCMCaseList({ onNavigate } = {}) {
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", background: COLORS.bg, fontFamily: "'DM Sans', 'Segoe UI', -apple-system, sans-serif" }}>
      ...
      <aside style={{ width: sW, background: COLORS.sidebar, display: "flex", flexDirection: "column", transition: "width 0.3s cubic-bezier(0.4,0,0.2,1)", flexShrink: 0, zIndex: 100, position: "relative" }}>
        ...
        <div style={{ padding: sidebarCollapsed ? "20px 16px" : "20px 24px", borderBottom: "1px solid rgba(255,255,255,0.06)", display: "flex", alignItems: "center", gap: 12, minHeight: 72 }}>
          ...
          {!sidebarCollapsed && <div style={{ overflow: "hidden" }}><div style={{ color: "#F8FAFC", fontWeight: 700, fontSize: 15, letterSpacing: "0.02em" }}>Fraud Case Manager</div><div style={{ color: "#64748B", fontSize: 11, letterSpacing: "0.03em" }}>Vaka Yöneticisi v1.0</div></div>}
        </div>
        ...
        <div style={{ padding: sidebarCollapsed ? "16px 8px" : "16px", borderTop: "1px solid rgba(255,255,255,0.06)" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, justifyContent: sidebarCollapsed ? "center" : "flex-start" }}>
            <div style={{ width: 34, height: 34, borderRadius: "50%", flexShrink: 0, background: "linear-gradient(135deg, #6366F1, #8B5CF6)", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontWeight: 600, fontSize: 13 }}>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                ...
                <tbody>
                  {REVIEW_REQUESTS.map(r => { const lc = cases.find(c => c.id === r.caseId); return (
                    <tr key={r.id} style={{ borderBottom: `1px solid ${COLORS.border}`, cursor: "pointer" }} onMouseEnter={e => e.currentTarget.style.background = "#FAFBFC"} onMouseLeave={e => e.currentTarget.style.background = "transparent"} onClick={() => lc && setDrawerCase(lc)}>
                      ...
                      <td style={{ padding: "12px 14px" }} onClick={e => e.stopPropagation()}><button style={{ padding: "6px 14px", borderRadius: 7, border: "none", background: COLORS.primary, color: "#fff", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>İncele</button></td>
                    </tr>); })}
                </tbody>
              </table>
            </div>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                ...
                <tbody>
                  {PENDING_APPROVALS.map(a => { const lc = cases.find(c => c.id === a.caseId); return (
                    <tr key={a.id} style={{ borderBottom: `1px solid ${COLORS.border}`, cursor: "pointer" }} onMouseEnter={e => e.currentTarget.style.background = "#FAFBFC"} onMouseLeave={e => e.currentTarget.style.background = "transparent"} onClick={() => lc && setDrawerCase(lc)}>
      ...
      {/* Overlays */}
      {drawerCase && <><div onClick={() => setDrawerCase(null)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.3)", zIndex: 999 }} /><CaseDrawer caseData={drawerCase} onClose={() => setDrawerCase(null)} /></>}
    </div>
  );
}

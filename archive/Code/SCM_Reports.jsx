...
export default function SCMReports({ onNavigate } = {}) {
  ...
  const [reportStatus,setReportStatus]=useState("idle");const [reportData,setReportData]=useState(null);
  ...
  const handleRun=()=>{if(!canRun||selectedCases.size===0)return;setReportStatus("loading");setTimeout(()=>{const d=previewCases.filter(c=>selectedCases.has(c.id));setReportData(d);setReportStatus("ready");showT("success",`"${selectedReport.name}" raporu oluşturuldu. ${d.length} vaka ile hazırlandı.`);},1500);};
  const handleEmail=()=>{if(reportStatus!=="ready")return;setReportStatus("sent");showT("info","Rapor e-posta adresinize gönderilecektir.");};
  const handleExport=()=>{if(reportStatus!=="ready")return;showT("success",`"${selectedReport.name}" Excel (.xlsx) olarak indiriliyor...`);};
  ...
  return (
    <div style={{display:"flex",minHeight:"100vh",background:C.bg,fontFamily:"'DM Sans','Segoe UI',-apple-system,sans-serif"}}>
      ...
      <aside style={{width:sW,background:C.sidebar,display:"flex",flexDirection:"column",transition:"width 0.3s cubic-bezier(0.4,0,0.2,1)",flexShrink:0,zIndex:100,position:"relative"}}>
        ...
        <div style={{padding:"12px 10px",borderTop:"1px solid rgba(255,255,255,0.06)",display:"flex",flexDirection:"column",gap:4}}>
          <button onClick={e=>{e.stopPropagation();setShowNotifications(!showNotifications);}} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 16px",borderRadius:8,border:"none",cursor:"pointer",width:"100%",textAlign:"left",background:showNotifications?"rgba(59,130,246,0.15)":"transparent",color:"#94A3B8",justifyContent:sidebarCollapsed?"center":"flex-start",position:"relative"}}>
            <span style={{position:"relative",display:"flex"}}><Icons.Bell/>{NOTIFICATIONS.filter(n=>!n.read).length>0&&<span style={{position:"absolute",top:-4,right:-4,width:8,height:8,borderRadius:"50%",background:"#EF4444",border:"2px solid #0F172A"}}/>}</span>
            {!sidebarCollapsed&&<span style={{fontSize:13,fontWeight:500}}>Bildirimler</span>}
          </button>
          {showNotifications&&<div style={{position:"fixed",left:sW+8,bottom:60,background:"#fff",borderRadius:12,border:`1px solid ${C.border}`,boxShadow:"0 12px 40px rgba(0,0,0,0.12)",width:340,zIndex:300}}>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",minWidth:0}}>
        <header style={{background:"#fff",borderBottom:`1px solid ${C.border}`,padding:"0 24px",display:"flex",alignItems:"center",justifyContent:"space-between",height:56,flexShrink:0}}>
          <div style={{display:"flex",alignItems:"center",gap:16}}>
            ...
            <div style={{display:"flex",gap:0,marginLeft:16,borderRadius:8,overflow:"hidden",border:`1px solid ${C.border}`}}>
              <button onClick={()=>setReportMode("standard")} style={{padding:"6px 16px",fontSize:12.5,fontWeight:600,border:"none",cursor:"pointer",background:reportMode==="standard"?C.primary:"#fff",color:reportMode==="standard"?"#fff":C.textSecondary}}>Standart Raporlar</button>
              <button onClick={()=>setReportMode("manual")} style={{padding:"6px 16px",fontSize:12.5,fontWeight:600,border:"none",cursor:"pointer",background:reportMode==="manual"?"#7C3AED":"#fff",color:reportMode==="manual"?"#fff":C.textSecondary}}>Manuel Rapor Oluşturma</button>
            </div>
          </div>
        ...
        <main style={{flex:1,overflow:"auto",padding:24}} onClick={()=>{setShowNotifications(false);setDomainMenuOpen(false);}}>
              ...
              {!selectedReport?<div style={{flex:1,background:"#fff",borderRadius:12,border:`1px solid ${C.border}`,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:40}}>
                ...
                <div style={{background:"#fff",borderRadius:12,border:`1px solid ${C.border}`,padding:"18px 22px"}}>
                  ...
                  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",paddingTop:14,borderTop:`1px solid ${C.border}`}}>
                    <div style={{fontSize:11,color:canRun?C.textSecondary:C.warning}}>{!canRun?"Tarih aralığı girilmeden rapor çalıştırılamaz.":isReady?`Rapor hazır — ${reportData?.length||0} vaka`:`${selectedCases.size} / ${previewCases.length} vaka seçili`}</div>
                    <div style={{display:"flex",gap:8}}>
                      ...
                      <button onClick={handleExport} disabled={!isReady} style={{display:"flex",alignItems:"center",gap:6,padding:"8px 16px",borderRadius:8,border:`1px solid ${isReady?C.success+"60":C.border}`,background:isReady?"#F0FDF4":"#F8FAFC",cursor:isReady?"pointer":"not-allowed",fontSize:13,fontWeight:500,color:isReady?C.success:C.textSecondary,opacity:isReady?1:0.4}}><Icons.Download/> Excel İndir</button>
                      <button onClick={handleEmail} disabled={!isReady} style={{display:"flex",alignItems:"center",gap:6,padding:"8px 16px",borderRadius:8,border:`1px solid ${isReady?C.primaryLight+"60":C.border}`,background:isReady?"#EFF6FF":"#F8FAFC",cursor:isReady?"pointer":"not-allowed",fontSize:13,fontWeight:500,color:isReady?C.primaryLight:C.textSecondary,opacity:isReady?1:0.4}}><Icons.Mail/> E-posta Gönder</button>
                    </div>
                  </div>
                </div>
                ...
                {/* Ready — download or send */}
                {isReady&&reportData&&<div style={{background:"#fff",borderRadius:12,border:`1px solid ${C.border}`,overflow:"hidden"}}>
                  <div style={{padding:"32px 40px",display:"flex",flexDirection:"column",alignItems:"center"}}>
                    <div style={{width:64,height:64,borderRadius:16,background:"#D1FAE5",display:"flex",alignItems:"center",justifyContent:"center",marginBottom:16}}><svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#059669" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg></div>
                    <div style={{fontSize:17,fontWeight:700,color:C.text,marginBottom:6}}>Rapor Hazırlandı</div>
                    <div style={{fontSize:13,color:C.textSecondary,marginBottom:20,textAlign:"center",lineHeight:1.5}}><strong>"{selectedReport.name}"</strong> raporu <strong>{reportData.length} vaka</strong> ile oluşturuldu.<br/>Yukarıdaki butonlarla Excel olarak indirin veya e-posta ile gönderin.</div>
                    <div style={{background:"#F8FAFC",borderRadius:10,border:`1px solid ${C.border}`,padding:"14px 20px",width:"100%",maxWidth:480}}>
                      <div style={{fontSize:11,fontWeight:600,color:C.textSecondary,textTransform:"uppercase",letterSpacing:"0.03em",marginBottom:10}}>Rapor Özeti</div>
                      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12}}>
                        <div><div style={{fontSize:11,color:C.textSecondary}}>Tarih Aralığı</div><div style={{fontSize:13,fontWeight:600,color:C.text,marginTop:2}}>{dateFrom} — {dateTo}</div></div>
                        <div><div style={{fontSize:11,color:C.textSecondary}}>Para Birimi</div><div style={{fontSize:13,fontWeight:600,color:C.text,marginTop:2}}>{currency==="original"?"Orijinal":currency}</div></div>
                        <div><div style={{fontSize:11,color:C.textSecondary}}>Vaka Sayısı</div><div style={{fontSize:13,fontWeight:600,color:C.text,marginTop:2}}>{reportData.length}</div></div>
                      </div>
                      <div style={{display:"flex",gap:8,marginTop:12,flexWrap:"wrap"}}>
                        {Object.entries(STATUS_MAP).map(([k,v])=>{const cnt=reportData.filter(r=>r.status===k).length;return cnt>0?<span key={k} style={{fontSize:11,padding:"3px 8px",borderRadius:4,background:v.bg,color:v.color,fontWeight:500}}>{v.label}: {cnt}</span>:null;})}
                        {Object.entries(SEVERITY_MAP).map(([k,v])=>{const cnt=reportData.filter(r=>r.severity===k).length;return cnt>0?<span key={k} style={{fontSize:11,padding:"3px 8px",borderRadius:4,background:v.bg,color:v.color,fontWeight:500}}>{v.label}: {cnt}</span>:null;})}
                      </div>
                    </div>
                    <button onClick={()=>{setReportStatus("idle");setReportData(null);}} style={{marginTop:16,fontSize:12,color:C.textSecondary,background:"none",border:"none",cursor:"pointer",textDecoration:"underline"}}>Yeni parametrelerle tekrar oluştur</button>
                  </div>
                </div>}
          ...
          {/* ══════ MANUEL RAPOR ══════ */}
          {reportMode==="manual"&&<div style={{display:"flex",gap:20,minHeight:"calc(100vh - 120px)"}}>

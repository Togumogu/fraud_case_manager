...
// MAIN COMPONENT
// ═══════════════════════════════════════════════════════════════
export default function SCMCaseDetail({ onNavigate, initialCase } = {}) {
  const baseCase = initialCase ? {
    ...CASE_DATA,
    id: initialCase.id,
    name: initialCase.name,
    status: initialCase.status,
    severity: initialCase.severity ?? CASE_DATA.severity,
    owner: initialCase.owner || CASE_DATA.owner,
    description: initialCase.description || CASE_DATA.description,
  } : CASE_DATA;
  ...
  // Fraud distribution
  const [bankShareVal, setBankShareVal] = useState(baseCase.bankShare);
  const [customerShareVal, setCustomerShareVal] = useState(baseCase.customerShare);
  ...
  return (
    <div style={{ display: "flex", height: "100vh", fontFamily: "'DM Sans',sans-serif", background: C.bg, overflow: "hidden", ...(darkMode ? {filter:"invert(1) hue-rotate(180deg)"} : {}) }}>
      ...
      <aside style={{ width: sW, minWidth: sW, height: "100vh", background: C.sidebar, display: "flex", flexDirection: "column", transition: "all .25s cubic-bezier(.4,0,.2,1)", zIndex: 50 }}>
        ...
        {!sidebarCollapsed && <div style={{ padding: "12px 20px", borderTop: "1px solid rgba(255,255,255,0.08)" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div onClick={() => setShowUserMenu(!showUserMenu)} style={{ width: 32, height: 32, borderRadius: 8, background: `linear-gradient(135deg,${C.primaryLight},#7C3AED)`, display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontWeight: 700, fontSize: 12, cursor: "pointer" }}>{user.name.split(" ").map(n => n[0]).join("")}</div>
            <div onClick={() => setShowUserMenu(!showUserMenu)} style={{ flex: 1, cursor: "pointer" }}><div style={{ color: "#fff", fontSize: 12.5, fontWeight: 600 }}>{user.name}</div><div style={{ color: "rgba(255,255,255,0.4)", fontSize: 10 }}>{user.roleLabel}</div></div>
            {/* Dark Mode Toggle */}
          ...
          {showUserMenu && (
            <div onClick={e => e.stopPropagation()} style={{ position: "fixed", bottom: 72, left: sW + 8, background: "#fff", borderRadius: 10, border: "1px solid #E2E8F0", boxShadow: "0 8px 24px rgba(0,0,0,0.12)", width: 180, zIndex: 400, overflow: "hidden" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        <main style={{ flex: 1, overflow: "auto", padding: "24px 28px" }} onClick={() => { setShowNotifPanel(false); setShowUserMenu(false); }}>
          {/* ── Case Header ── */}
